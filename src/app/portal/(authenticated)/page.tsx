...
export default async function PortalIndex() {
  ...
  if (error || !projects) {
    return <div className="p-6 text-gray-500">Failed to load projects.</div>
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((p) => (
          <Link href={`/portal/${p.slug}`} key={p.slug} className="block group">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm group-hover:border-brand-primary/50 group-hover:shadow-md transition-all h-full flex flex-col relative overflow-hidden">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{p.status.replace('_', ' ')}</span>
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-2">{p.title}</h3>
              <p className="text-gray-500 text-xs flex-1 mb-6">{p.description}</p>
